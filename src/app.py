...
def app_layout(df):
    ...
    return html.Div([
            ...
            dcc.Graph(
                ...
                figure={
                    'data': [{
                        ...
                        'text': [f"{name}, {infomail}" for name, infomail in zip(df.Name, df.Infomail)],
                    }],
                    'layout': {
                        'mapbox': {
                            'accesstoken': (
        ...
        html.Div([
            html.Div(children=html.Pre(id='lasso', style={'overflowY': 'scroll', 'height': '25vw'}))],
        ...
        html.Div([
            ...
            style={'display': 'inline-block', 'verticalAlign': 'top', 'marginTop': '3vw', 'overflowY': 'scroll', 'height': '15vw'}
        )
    ], className="row")
...
def download_state(n_clicks, selectedData):
    if n_clicks is not None:
        if selectedData is not None:
            df_selected = pd.DataFrame(selectedData['points'])
            point_indices = df_selected["pointIndex"]
            all_selected = df[df.index.isin(
                point_indices) | (df['is_contacted'] == True)]
            all_selected.loc[:, 'is_contacted'] = True
            csv_string = all_selected.to_csv(encoding='utf-8', sep=",")
